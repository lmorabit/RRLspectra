...
;	version 1.0 01/23/2013: Initial version
	...
	specfile = '/net/leubeek/data1/m82/sgstack/gsgstack_' + sigmastr + '.total.spec.dat'
	readcol, specfile, frequency, spectra, format='D,D,D'
	...
	;redshiftrange = (dindgen(2))/1d5 ;; to test the algorithm
	...
	for kk=0,n_elements(linevelocities)-1 do begin
		...
		for jj=0,n_elements(redshiftrange)-1 do begin
			...
			ts = 'templates/template_spec_'+sigmastr+'_z'+cgnumber_formatter(redshift,decimal=2)+'_v'+strtrim(string(uint(vwidth)),2)+'.dat'
		...
		plotfile = 'crosscorr/crosscorr_' + sigmastr + '_v' + cgnumber_formatter(vwidth,decimal=0) + 'eps'
	...
	logfile = 'crosscorr_'+sigmastr+'.dat'
	...
	plotfile = 'crosscorr_'+sigmastr+'.eps'
	;; determine yrange
